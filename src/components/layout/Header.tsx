...
import { Search, Plus, Moon, Sun, RefreshCw, User, LogOut, UserPlus, LogIn, Sparkles, ChevronDown } from 'lucide-react';
...
export default function Header() {
  ...
  return (
    <>
      <header className="h-16 px-4 sm:px-6 bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-800 flex items-center justify-between z-20 shrink-0">
        {/* Global Search Bar */}
        <div className="flex items-center gap-3 flex-1 max-w-md">
          <div className="relative w-full">
            ...
            <input
              type="text"
              placeholder="Search contacts, deals, invoices, tasks... (Press ⌘K)"
              className="w-full pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-800/80 text-xs sm:text-sm text-slate-900 dark:text-slate-100 rounded-xl border border-transparent focus:border-indigo-500/50 focus:bg-white dark:focus:bg-slate-900 focus:outline-none transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {contacts.length === 0 && (
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 rounded-xl border border-indigo-200 dark:border-indigo-800 transition-colors"
            >
              <Sparkles size={13} />
              Load Sample Data
            </button>
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-xs sm:text-sm font-medium rounded-xl shadow-sm shadow-indigo-500/20 transition-all"
            >
              <Plus size={16} />
